...
pdf(file = "~/Documents/MBARI/Lipids/cteno-lipids-2020/figs/2/Fig2_top13_var1.pdf", width = 110/25.4, height = 320/25.4)
ggplot() +
  facet_grid(
    cols=vars(env_var),
    rows=vars(id),
  ...
  geom_rect(
    ...
    data=fig2_eo_fits %>% mutate(sig = (p<= (0.05/20))),
  ...
  # total abundance
  geom_boxplot(
    data = fig2_eo_data %>% filter(env_var == "tot"),
  ...
  #geom_text(data = fig2_eo_data, aes(x=env_val, y=frac_molar, label=eid, color=sprep)) + #TEST
  # PGLS trendline
  geom_abline(data = fig2_eo_fits, aes(slope=b, intercept=c)) +
  # scales and theme
  theme_pubr() +
  theme(
