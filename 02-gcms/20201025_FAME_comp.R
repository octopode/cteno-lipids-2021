...
source(here("02-gcms", "20200821_FAME_comp_funcs.R"))
# load data
load(here("02-gcms", "20210312_comp_unks.RData"))

# analysis parameters
shal = 200 # "shallow" samples are <= 200 m
cold = 7.5 # "cold" samples are <= 7.5°C
...
file_tree <- here("01-phylo", "20200819_iq.tree")
...
file_meta <- here("00-metadata", "old", "20201022_lipid_metadata.tsv")
dir_envi <- here("00-metadata")

# read VARS-format files
envi_data <- list.files(path = dir_envi, full.names = T, pattern = ".tsv") %>%
  lapply(
    function(file_data){
...
samp_comp <- comp_unks %>%
  ...
  filter(roi == last(roi)) %>%
  distinct(eid, id, sp, tissue, depth_col, temp_col, do_col, frac_molar) %>%
...
## species by habitat
pdf(file = "~/Documents/MBARI/Lipids/20201122_hplc/specimens.pdf", width = 6, height = 4.6)
samp_comp_fastats %>%
  group_by(eid) %>%
  filter(
    eid %in% c(
