...
pdf(file = "~/Documents/MBARI/Lipids/cteno-lipids-2020/Fig2_var15.pdf", width = 185/25.4, height = 135/25.4)
ggplot() +
  facet_grid(
    cols=vars(id.x),
    rows=vars(id.y),
...
pgls_chain_temp <- samp_fastats_phylo %>%
  filter(depth_col <= shal) %>%
  pgls_ou(., tree_rooted, chain ~ temp_col, branch_mult = 1E13)
...
tree_pruned <- prune_to_one(samp_fastats_phylo, tree_rooted)
fastats_phylo_summary <- samp_fastats_phylo %>%
  group_by(sp) %>%
  summarize(
    count = n(),
